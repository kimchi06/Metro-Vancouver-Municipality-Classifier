...
def conversion(gps, ref):
    degrees = gps[0] + gps[1] / 60 + gps[2] / 3600
    # Flip if south or west
    if (ref == "S" or ref == "W"):
        return -degrees
    else: 
        return degrees
...
def exif_extract(image_directory):
    coordinates = None

    with open(image_directory, 'rb') as src:
        image = Image(src)
        if (image.has_exif == True):
            gps_lat = image.gps_latitude
            gps_lon = image.gps_longitude
            ref_lat = image.gps_latitude_ref
            ref_lon = image.gps_longitude_ref

            latitude = conversion(gps_lat, ref_lat)
            longitude = conversion(gps_lon, ref_lon)

            coordinates = [[latitude, longitude]]
        else:
            raise Exception("Image has no coordinate data.")
